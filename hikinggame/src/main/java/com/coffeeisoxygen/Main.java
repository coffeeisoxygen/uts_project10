...
import com.coffeeisoxygen.model.classes.PlayerState;
import com.coffeeisoxygen.model.factory.TileFactory;
import com.coffeeisoxygen.model.interfaces.IMapLoader;
import com.coffeeisoxygen.model.interfaces.ITileFactory;
...
public class Main {
    public static void main(String[] args) {
        ...
        Board board = defaultMapLoader.loadMap();

        // Create a player
        System.out.println("Enter player name:");
        try (Scanner scanner = new Scanner(System.in)) {
            String playerName = scanner.next();
            Player player = new Player(playerName, 100, new Coordinate(0, 0)); // Starting energy and position
            ...
            System.out.println("Enter movement plan (e.g., 'RRUUDDLL'): ");
            String movementPlan = scanner.next();
            System.out.println("Executing movement plan...");
            game.executeMovementPlan(movementPlan.toUpperCase()); // Convert to uppercase when passing

            // Print player's position
            Coordinate playerPosition = PlayerState.getPosition();
            System.out.println("Player's position: (" + playerPosition.getX() + ", " + playerPosition.getY() + ")");
